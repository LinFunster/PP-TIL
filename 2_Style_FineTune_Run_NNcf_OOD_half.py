...
"""
nn+cost function+traj loss
"""
# 设置打印选项
np.set_printoptions(suppress=True, formatter={'float_kind': '{:.4f}'.format})


def LfD_Loss(cost_function_weights, planner, E_expert_features,control_var, prediction, ground_truth,ref_line,current_state):
    # IRL
    planner_inputs = {
        "control_variables": control_var.view(-1, 100),
    ...
    control_var = final_values["control_variables"].view(-1, 50, 2)
    ...
    # print(info_str)
    loss_value_list = [loss.detach().item(),IRL_loss.detach().item(),traj_loss.detach().item(),plan_cost.item()]
...
def train_task():
    ...
    max_iterations, opt_step_size= 2, 0.4
    planner = MotionPlanner(trajectory_len, feature_len, device, max_iterations, opt_step_size) # , max_iterations, opt_step_size
    ...
    for i in range(3): # style
        ...
        predictors[i].train()
        paramModels[i] = [RewardFunction2().to(device) for i in range(2)]
    ...
    epoch_count = 1
    while epoch_count<args.train_epochs+1:
        for style_i in range(3):
            for epoch in range(epoch_count, args.train_epochs+1):
                epoch_loss_list = []
                epoch_irl_loss_list = []
                epoch_traj_loss_list = []
                epoch_plan_cost_list = []
                epoch_metrics_list = []
                ...
                for behavior_i in range(2):
                    ...
                    # print(control_var) 
                    cost_function_weights = paramModels[style_i][behavior_i-1]()
                    ###################################################################################################
                    """learn process""" 
                    learn_loss, learn_traj, loss_value_list = LfD_Loss(cost_function_weights,planner,E_expert_features,control_var,prediction,ground_truth,ref_line,current_state)
                    ...
                    epoch_plan_cost_list.append(loss_value_list[3]) 
                    epoch_metrics_list.append(learn_metrics) 

                sys.stdout.write(f"\rProgress:[{epoch:>4d}/{args.train_epochs:>4d}]  Loss:{np.mean(epoch_loss_list):>.5f}  Pred-Loss:{np.mean(epoch_pred_loss_list):>.5f}  IRL-Loss:{np.mean(epoch_irl_loss_list):>.5f}  Traj-Loss:{np.mean(epoch_traj_loss_list):>.5f}  Plan-Cost:{np.mean(epoch_plan_cost_list):>.5f}  {(time.time()-start_time)/epoch:>.5f}s/sample") 
                sys.stdout.flush()
                ...
                if epoch%show_epoch_N == 0: 
                    ...
                    logging.info(f"Progress:[{epoch:>4d}/{args.train_epochs:>4d}]  Loss:{np.mean(epoch_loss_list):>.5f}  Pred-Loss:{np.mean(epoch_pred_loss_list):>.5f}  IRL-Loss:{np.mean(epoch_irl_loss_list):>.5f}  Traj-Loss:{np.mean(epoch_traj_loss_list):>.5f}  Plan-Cost:{np.mean(epoch_plan_cost_list):>.5f}  {(time.time()-start_time)/epoch:>.5f}s/sample")
                    ...
                    log = {
                            ...
                            'epoch_plan_cost_list':np.mean(epoch_plan_cost_list), 
                            # metrics
                    ...
                    if epoch == show_epoch_N: 
                        with open(f'./training_log/{log_name}/train_log_{style_i}.csv', 'w') as csv_file: 
                            writer = csv.writer(csv_file)
                            writer.writerow(log.keys())
                            writer.writerow(log.values())
                    else:
                        with open(f'./training_log/{log_name}/train_log_{style_i}.csv', 'a') as csv_file: 
                            writer = csv.writer(csv_file)
                            writer.writerow(log.values())
                ...
                if epoch%100 == 0:
                    ...
                    # save cost function
                    cost_function_weights_list = []
                    for behavior_i in range(2):
                        cost_function_weights_np = paramModels[style_i][behavior_i]().cpu().detach().numpy()
                        logging.info(f"behavior_i: {behavior_i}  style_i: {style_i}")
                        logging.info(cost_function_weights_np)
                        cost_function_weights_list.append(cost_function_weights_np)
                    np.save(save_path+f"CostFunction_style_{style_i}_{epoch}",np.vstack(cost_function_weights_list))
                    if style_i==2:
                        epoch_count+=100
                    break
